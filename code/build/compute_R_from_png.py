...
def print_matrix_stats(name, arr):
    """输出矩阵的统计特性"""
    print(f"\n[{name}]")
    print(f"  - 形状: {arr.shape}")
    print(f"  - 数值范围: [{np.min(arr):.4f}, {np.max(arr):.4f}]")
    print(f"  - 平均值: {np.mean(arr):.4f}")
    print(f"  - 标准差(噪声水平): {np.std(arr):.4f}")
    # 计算变异系数 (CV)，这是衡量信噪比最直观的指标
    cv = (np.std(arr) / np.mean(arr)) * 100 if np.mean(arr) != 0 else 0
    print(f"  - 变异系数 (Std/Mean): {cv:.2f}%")
...
def main():
    ...
    # === 第四步：计算 R 图像 ===
    # R = A_Low / A_High = -ln(T_Low) / -ln(T_High) = log_l_abs / log_h_abs
    # R 值反映材质的有效原子序数，用于区分不同材料（如煤与矸石）
    R = (log_l_abs + bix_1_value) / (log_h_abs + bix_1_value)
    print_matrix_stats("R 矩阵 (材质识别值)", R)

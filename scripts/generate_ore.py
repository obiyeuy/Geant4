...
def generate_tessellated_ore(
    ...
    if not has_triangular_facet and not has_create_tess:
        if debug:
            print("[tess-debug] both TriangularFacet and createTessellatedSolid unavailable.")
        return None
    ...
    if not polygons:
        if debug:
            print(f"[tess-debug] failed to build tessellated solid: {reject_stats}")
        return None
    if debug:
        print(f"[tess-debug] build succeeded with {len(polygons)} facets; rejects={reject_stats}")
...
def create_rugged_ore_gdml(
    ...
    if mode == "tessellated":
        if tess_debug:
            print(
                "[tess-debug] TriangularFacet probe: "
                f"ok={tri_probe['ok']}, "
        ...
        if ore_solid is None:
            print(
                ...
                f"原因: {tri_probe['reason']}。"
                "需要 scipy + TessellatedSolid + TriangularFacet；"
                "默认禁用不稳定 createTessellatedSolid 回退，自动回退到 csg 模式。"
            )
            mode = "csg"
    ...
    w.write(output_path)
    print(
        f"成功生成棱角矿石模型: {output_path} "
        f"(模式: {mode}, 材质: {'mix' if mix_items else matrix_material}, "
        f"凸起块: {num_lumps}, 切割面: {num_cuts}, 尺寸标度: {base_scale})"
    )
...
if __name__ == "__main__":
    ...
    if args.probe_triangularfacet:
        ...
        print("[triangularfacet-probe]")
        print(f"ok={probe['ok']}")
        print(f"has_scipy={probe['has_scipy']}")
        print(f"has_tessellated_solid={probe['has_tessellated_solid']}")
        print(f"has_triangular_facet={probe['has_triangular_facet']}")
        print(f"has_create_tessellated_solid={probe['has_create_tessellated_solid']}")

...
- `--num-samples`: 生成样本数量
- `--ore-ratio`: 矿石样本占比
- `--geant-exec`: Geant4 可执行文件路径
- `--stages`: 可选执行阶段（generate/simulate/build/train）
...
- `G4_OUTPUT_DIR`: 设置 Geant4 输出目录（由 `scripts/pipeline/run_full_pipeline.py` 自动设置）
...
# 4. 运行自动化全流程管线
cd ../..
python3 scripts/pipeline/run_full_pipeline.py --num-samples 5
```
...
可以在 `scripts/pipeline/stages.py` 中扩展并行模拟策略，提高吞吐。

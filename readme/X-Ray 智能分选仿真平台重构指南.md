...
### 4.2 批次管理与运行 (`scripts/pipeline/run_full_pipeline.py`)

负责调度整个模拟流程。

- **目录结构**： `data/raw/batch_{batch_id}/sample_{id}_{ore|waste}/`
- **执行步骤**：
  1. 创建样本文件夹。
  2. 调用生成器生成 `ore.gdml` 到该文件夹。
  3. 生成启动宏 `launch_sample.mac`，写入 `/Xray/det/loadGDML .../ore.gdml` 并执行 `master.mac`。
  4. 设置 `os.environ["G4_OUTPUT_DIR"]` 为样本文件夹路径。
  5. 调用 `subprocess.run([CZT, launch_sample.mac])`。
  6. 写入 `info.json`：记录类别、材质配比、几何参数（作为 Ground Truth）。
  7. 执行 R值数据集构建与 EfficientNet 训练（可通过阶段开关控制）。
